...
var PdfHighlighter = function (_PureComponent) {
  ...
  function PdfHighlighter() {
    ...
    return _ret = (_temp = (_this = _possibleConstructorReturn(this, _PureComponent.call.apply(_PureComponent, [this].concat(args))), _this), _this.state = {
      ...
      var textLayerContent = document.querySelectorAll('.textLayer div:not(.Highlight__part)');
      for (let i = 0; i < textLayerContent.length; i++) {
        var textDivElement = textLayerContent[i];

        var nextTextDivElement = textLayerContent[i + 1];

        if (!nextTextDivElement) {
          return
        }

        var textDivElementTop = parseInt(textDivElement.style.top);
        var textDivElementHeight = textDivElement.offsetHeight;
        var nextTextDivElementTop = parseInt(nextTextDivElement.style.top);

        // for better handling when multicolumn
        if (textDivElementTop === nextTextDivElementTop) {
          for (var n = i; n < textLayerContent.length; n++) {
            nextTextDivElement = textLayerContent[n + 1];
            nextTextDivElementTop = parseInt(nextTextDivElement.style.top);
        ...
        var commonPadding = nextTextDivElementTop - (textDivElementHeight + textDivElementTop);

        if (commonPadding) {
          // textDivElement.style.border = '1px solid black';
          textDivElement.style.paddingBottom = commonPadding + 'px';
        }
      }
      ...
      var textLayerContent = document.querySelectorAll('.textLayer div:not(.PdfHighlighter__highlight-layer)');
      for (var i = 0; i < textLayerContent.length; i++) {
      ...
      for (var _i = 0; _i < textLayerContent.length; _i++) {
        textLayerContent[_i].classList.remove("widthPaddingUnset");
      }
